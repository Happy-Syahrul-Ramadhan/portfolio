"use client"

import { useTransition } from "react"
import { Eye, EyeOff, Trash2 } from "lucide-react"
import { deleteBlog, toggleBlogPublish } from "@/app/actions/blog"
import { useToast } from "@/app/components/ToastProvider"

interface BlogActionsProps {
  blogId: string
  blogTitle: string
  isPublished: boolean
}

export default function BlogActions({ blogId, blogTitle, isPublished }: BlogActionsProps) {
  const [isPending, startTransition] = useTransition()
  const { showToast } = useToast()

  const handleTogglePublish = () => {
    startTransition(async () => {
      try {
        await toggleBlogPublish(blogId, isPublished)
        showToast(
          isPublished ? `"${blogTitle}" unpublished` : `"${blogTitle}" published`,
          "success"
        )
      } catch (error) {
        showToast("Failed to update blog post", "error")
      }
    })
  }

  const handleDelete = () => {
    if (!confirm(`Delete "${blogTitle}"?`)) return

    startTransition(async () => {
      try {
        await deleteBlog(blogId)
        showToast(`"${blogTitle}" deleted`, "success")
      } catch (error) {
        showToast("Failed to delete blog post", "error")
      }
    })
  }

  return (
    <>
      <button
        onClick={handleTogglePublish}
        disabled={isPending}
        title={isPublished ? "Unpublish" : "Publish"}
        className="inline-flex items-center justify-center rounded-md text-sm transition-colors hover:bg-accent h-9 w-9 text-muted-foreground hover:text-foreground disabled:opacity-50"
      >
        {isPublished ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
      </button>

      <button
        onClick={handleDelete}
        disabled={isPending}
        title="Delete"
        className="inline-flex items-center justify-center rounded-md text-sm transition-colors hover:bg-destructive/10 text-destructive h-9 w-9 disabled:opacity-50"
      >
        <Trash2 className="h-4 w-4" />
      </button>
    </>
  )
}
